...
func get_meshs(center:Vector3, nbr_sub_regions:int,start_offset:Vector2) -> Array[MeshInstance3D]:
	...
	for i in range(1,nbr_sub_regions+1):
		...
		var triangle_mesh:MeshInstance3D = draw_multi_line(_get_triangle_array(center,triangle_size,start_offset))
		meshs.append(triangle_mesh)
	var triangle_mesh:MeshInstance3D = draw_multi_line(_get_triangle_array(center,1,start_offset))
	meshs.append(triangle_mesh)
	
	return meshs

func _get_triangle_array(center:Vector3,x:float,start_offset:Vector2) -> Array[Vector3]:
