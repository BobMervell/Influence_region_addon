...
func get_distance_magnitude(center:Vector3,pos:Vector3,nbr_sub_regions:int) -> float:
	...
	#if pos_2D.distance_squared_to(center_2D) > circles[-1].radius:
		#return 0
	var magnitude:float = nbr_sub_regions - find_first_greater_than(pos_2D)
	if nbr_sub_regions > 0: magnitude = magnitude/float(nbr_sub_regions)
	return magnitude

func find_first_greater_than(pos_2D: Vector2) -> float:
	var low: int = 0
